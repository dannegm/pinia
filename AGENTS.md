...
- Custom map style: `src/constants/map-style.json`, passed as `<Map styles={{ light: customMapStyle }} .../>` in `map-shell.jsx`. It's a hand-edited copy of CARTO's Positron style (same vector source/tiles, `https://tiles.basemaps.cartocdn.com/vector/carto.streets/v1/tiles.json`) — **not** a Google Maps JS style; Google's `featureType`/`elementType`/`stylers` JSON format the owner initially wanted to reuse is incompatible with MapLibre's style spec (sources/layers/paint), so the intent was translated by hand instead of imported. Colors: landscape `#f3f4f4`, water `#7fc8ed`, parks `#83cead`, roads white, highways `#fee379`, administrative boundaries/place-name text `#5b6571` at reduced opacity. Labels: all `place`/`transportation_name`/`water_name`/`waterway` (administrative names, city/town/country/state names, street names, water names) stay visible; POI/establishment labels (`poi_stadium`, `poi_park`) and house numbers are hidden. Every symbol layer has `icon-image`/`icon-color` stripped — text-only labels, no icons anywhere (including the small city-dot icons Positron ships by default). Building fill layers are hidden too (man-made landscape, off). Regenerate by re-fetching the Positron style JSON and re-applying the same per-layer-id rules if CARTO changes their base style meaningfully.
