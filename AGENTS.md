...
- Editing a place: **implemented**, `src/components/place-form.jsx` flies the map to the place's original coordinates (`map.flyTo({center: [initialCoords.lng, initialCoords.lat], zoom: 16, duration: 800})`) once on mount in edit mode (`mode === 'edit'`), on both desktop and mobile — a plain mount-only `useEffect(() => {...}, [])` (same "run once, don't re-track deps" convention already used by `useNtfy`), so the marker being edited is always visible/reachable for dragging regardless of where the map camera happened to be beforehand. `PlaceForm` also renders a mobile-only (`sm:hidden`) `Alert` (`src/ui/alert.jsx`, `default` variant) as the first item in its `FieldGroup`, right above the "Nombre" field, explaining the despejar-mapa button — icon is the same `Maximize`+nested-`Pin` composite as the header button itself, for visual continuity, shown on both `/places/new` and `/places/$placeId/edit` since `PlaceForm` is shared.
