...
- Search by place name, category, and address (`/search`, free-text). Quick-access category filter chips: **implemented**, `src/components/category-filter-chips.jsx` — multi-select toggle chips (one per category, icon+color), state synced to the URL via nuqs (`useCategoryFilter`, `?categories=id1,id2`). Wired into `/places`; not yet on `/search` (the two filtering mechanisms are separate for now — text search doesn't consider the chip selection).
