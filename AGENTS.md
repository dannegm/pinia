...
- Routing/directions: **implemented.** OpenRouteService (`VITE_ORS_API_KEY` env var, free tier, no credit card) computes route geometry via `src/queries/route.js`'s `routeQuery(origin, destination)` (plain `fetch` against `/v2/directions/driving-car`, not Supabase — still follows the query-factory convention). mapcn's `MapRoute` (`src/ui/map.jsx`) only renders the resulting line, it does not calculate routes itself.
...
- Custom map style: **currently disabled** — `map-shell.jsx`'s `<Map>` uses mapcn's plain default light style (CARTO Positron) again, no `styles` prop override. The owner went through several rounds trying to translate Google Maps JS style JSON (`featureType`/`elementType`/`stylers`) references into a custom MapLibre style and eventually decided to drop it ("me doy por vencido") — but explicitly asked not to delete any of the work, since the tooling is reusable if this comes back later.
  - Everything from that effort is preserved: `src/constants/map-style-base.json` (pristine CARTO Positron, never edited), `src/constants/map-style-sources/*.json` (every Google style JSON pasted, saved verbatim, in order), and `src/constants/map-style.json` (the last generated output — not wired into the app anymore, but still buildable).
  - The `/google-to-maplibre-style` skill (`.agents/skills/google-to-maplibre-style/`) does the actual translation: a `mapping.cjs` table (Google featureType/elementType → CARTO Positron layer ids, since Google's and MapLibre's style formats are fundamentally incompatible) plus `convert.cjs`, which always rebuilds `map-style.json` from the pristine base + every source file replayed in order — never edits its own output, which is what caused compounding drift in the hand-edited attempts before the skill existed. See the skill's `SKILL.md` for the encoded rules (e.g. a blanket `"all"` selector never shifts saturation/lightness — that drifts unrelated zones) and known unsupported zones (`transit` has no layer in CARTO's tiles at all, several `poi.*` subtypes aren't pre-defined layers, etc.).
  - To resume: re-add `styles={{ light: customMapStyle }}` (import from `@/constants/map-style.json`) to `<Map>` in `map-shell.jsx`, and use the skill for any further style pastes instead of hand-editing.
...
- Custom variants and utilities, in separate CSS files imported into `index.css` (proposed split, confirm filenames if it matters):
  ...
  ```
  The `page-*` variants shown are examples from the source project — update them to match Pinia's actual routes once they exist.
...
- `cache` (localStorage helper): source at `/Users/danielgarcia/Desktop/Workspace/bins/src/services/cache.js` → goes in `src/helpers/cache.js` here. Rename its `STORAGE_KEY` from `'bins:cache'` to a Pinia-scoped key (e.g. `'pinia:cache'`) — never reuse the original verbatim.
- `settings` (localStorage-backed, path-based, cross-tab-synced settings store — backs the `SettingsProvider`/local `useSettings` hook): source at `/Users/danielgarcia/Desktop/Workspace/bins/src/services/settings.js` → goes in `src/helpers/settings.js` here. Rename `STORAGE_KEY` from `'bins:settings'` to `'pinia:settings'`. Depends on `src/helpers/objects.js` (`getByPath`/`setByPath`, portable as-is, source `/Users/danielgarcia/Desktop/Workspace/bins/src/helpers/objects.js`, itself depending on `trim` from `src/helpers/strings.js` — check that file before porting) and `src/constants/default-settings.js` (the `bins` version is entirely bins-specific — port only the *pattern*, not its contents; Pinia's actual defaults are undecided, ask the owner).
...
- Supabase client factory, already Pinia-scoped, goes in `src/helpers/supabase.js`:
  ```js
