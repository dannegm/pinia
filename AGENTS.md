...
- Each place stores a free-text address AND lat/lng coordinates. On non-touch devices, pins are placeable via drag-and-drop on the map (mapcn's `Markers` supports this natively via `draggable`/`onDrag`) or by typing coordinates directly. On touch devices (`useIsTouchDevice()`, `src/hooks/use-is-touch-device.js` — wraps `useMediaQuery('(pointer: coarse)')`), dragging a small marker with a fingertip is imprecise, so the interaction inverts: the pin stays fixed at the visual center of the map and the person pans the map underneath it instead — `src/components/center-pin.jsx`'s `CenterPin` renders the pin as a plain absolutely-positioned `<div>`, portaled via `createPortal` straight into `map.getContainer()` — it's declared inside `PlaceForm`, which nests inside the panel/sheet's own DOM (`PanelContainer`'s `z-[120]` box with its own collapse `translateY`), so a plain, non-portaled div there would inherit that stacking (rendering on top of the panel instead of the map) and slide off-screen with the panel's collapse transform; portaling escapes that subtree and lands the pin as a sibling of the map canvas instead, stacking below the panel automatically since the panel sets an explicit z-index and this div doesn't. It's not a `MapMarker` either, which would move with lng/lat — instead it's inset by the map's *live* padding (`map.getPadding()` — the same source `panel-container.jsx` writes to for rail/panel compensation, kept as the single source of truth instead of re-deriving from `usePanelOffset()`) and centered inside that inset box, so it lines up with wherever `map.getCenter()` actually projects to on screen. It syncs coordinates on the map's native `move` event (`onCoordsChange(map.getCenter())`) and tracks the map's own `dragstart`/`dragend` events (user panning the map itself, not a marker drag) to report `isPanning` back to its children via a render-prop (`children(isPanning)`) so each caller can animate its own marker — `PinGlyph` (`src/components/pin-glyph.jsx`, extracted out of `PinDrop` so both the drag and pan-to-center paths render an identical glyph) lifts on `isPanning` in create mode, the category-colored circle marker scales up slightly in edit mode. `PlaceForm` (`src/components/place-form.jsx`) branches both modes on `isTouch`: create mode swaps `PinDrop` for `CenterPin`+`PinGlyph`; edit mode swaps the draggable `MapMarker` for `CenterPin` wrapping the same category-icon circle markup.
- Editing a place: **implemented**, `src/components/place-form.jsx` flies the map to the place's original coordinates (`map.flyTo({center: [initialCoords.lng, initialCoords.lat], zoom: 16, duration: 800})`) once on mount in edit mode (`mode === 'edit'`), on both desktop and mobile — a plain mount-only `useEffect(() => {...}, [])` (same "run once, don't re-track deps" convention already used by `useNtfy`), so the marker being edited is always visible/reachable regardless of where the map camera happened to be beforehand (drag on non-touch, pan-to-center on touch — see above). `PlaceForm` also renders a mobile-only (`sm:hidden`) `Alert` (`src/ui/alert.jsx`, `default` variant) as the first item in its `FieldGroup`, right above the "Nombre" field, explaining the despejar-mapa button — icon is the same `Maximize`+nested-`Pin` composite as the header button itself, for visual continuity, shown on both `/places/new` and `/places/$placeId/edit` since `PlaceForm` is shared. Its copy branches on `isTouch` too (not on the `sm:hidden` breakpoint, which is a separate, unrelated condition about viewport width) — "mueve el mapa para posicionar el pin" on touch vs. "mover el pin en el mapa" otherwise — so a narrow non-touch window (e.g. a resized desktop browser) still gets the drag-accurate copy.
...
- "Despejar mapa" on mobile `/places/new` and `/places/$placeId/edit`: **implemented** — since the mobile sheet can grow up to `max-h-[85dvh]` and fully covers the map, dragging the new place's pin (a `PinDrop`/draggable `MapMarker` underneath) was impossible on mobile. `src/pages/places-new.jsx` and `src/pages/places-edit.jsx` each pass a button, hidden on desktop via `sm:hidden` (not a `useMediaQuery`/JS conditional — plain CSS is the right tool here, matching the mobile-unprefixed/`sm:`-for-desktop convention), into `PanelHeader`'s `action` slot (right side of the title row) that toggles local `collapsed` state and emits a `panel:collapse` bus event (`useEvents().emit`, not `useEmitter`, since the emitted boolean must be the freshly-computed next value, not a value bound at hook-call time) carrying the new boolean. `src/components/panel-container.jsx` listens (`useListener('panel:collapse', setCollapsed)`) and, only in its mobile branch, feeds the collapse into the *same* `--dock-y` transform already used for drag-to-dismiss, rather than touching `top`/`bottom`/height at all: `dockY = collapsed ? calc(${dockDrag.y}px + 100% - ${COLLAPSED_HEIGHT}) : ${dockDrag.y}px` (`COLLAPSED_HEIGHT = '30dvh'`) — `100%` inside a `translateY()` resolves against the dock's own rendered height, so this is a pure paint-time shift (the dock's actual box size never changes, avoiding the earlier `top`+`bottom` approach that technically left the height unchanged as a CSS property but still changed the *computed* height, which was the real objection). Because the transform moves the whole dock (sheet + bottom `<nav>` together, same as an ordinary dismiss-drag already does), the bottom nav also slides mostly off-screen while collapsed — accepted as consistent with existing drag behavior rather than kept independently visible. `PanelContainer` resets `collapsed` to `false` on every `pathname` change so it never leaks into other panels. The icon is a composite built via lucide-react's icon-nesting feature (a child icon rendered inside a parent, positioned with `size`/`x`/`y`/`absoluteStrokeWidth` inside the parent's 24×24 viewBox — see `https://lucide.dev/guide/react/advanced/combining-icons`): `Maximize` (the 4-corner-bracket icon, same shape family as `Scan` — Lucide's own combining-icons example — not `Maximize2`, which is a diagonal-arrows icon) as the outer frame in both states, with `Pin` nested inside when open (not collapsed) or `Pencil` nested inside when collapsed, `size={12} x={6} y={6}` to center it. The outer icon inherits the button's `text-foreground/70`; the nested icon gets its own `className='text-foreground'` (matching `PanelHeader`'s title color) to read as a distinct, slightly bolder inner glyph. Each page keeps its own local `collapsed` state and emits the same shared `panel:collapse` event — there's no cross-page sync, but that doesn't matter since `PanelContainer` also resets `collapsed` on every `pathname` change.
...
- Keep this file, `CLAUDE.md`, and any `/docs/*.md` in sync after every significant change (there's a `sync-instructions` skill for keeping a group of instruction files identical — use it when applicable).
